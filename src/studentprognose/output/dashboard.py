...
class DashboardBuilder:
    def __init__(
        ...
        self.xgb_classifier_training_log: dict | None = None
        self.xgb_regressor_training_log: dict | None = None
    ...
    def _get_actual_trend(self, programme: str) -> pd.DataFrame | None:
        ...
        mask = (dc["Croho groepeernaam"] == programme) & (
        ...
        if self.predict_week is not None:
            ...
            agg = agg[
                agg["Weeknummer"].apply(lambda w: _week_sort_key(int(w)) <= pw_key)
            ]
    ...
    def _hist_accuracy(self) -> pd.DataFrame | None:
        ...
        hist_years = sorted(
            y for y in dc["Collegejaar"].unique() if y < self.prediction_year
        )
        ...
        for yr in hist_years:
            for prog in dc["Croho groepeernaam"].unique():
                ...
                act = sc[(sc["Croho groepeernaam"] == prog) & (sc["Collegejaar"] == yr)]
...
{body}
    ...
    def _build_kpi_cards(self, mape_cols: list[str]) -> str:
        ...
        for prog in programmes:
            ...
            col_means = [
                sub[c].dropna().mean() for c in valid_cols if not sub[c].dropna().empty
            ]
            if col_means:
        ...
        avg_cls = (
            "good" if avg_mape <= 0.10 else ("warn" if avg_mape <= 0.25 else "bad")
        )
  ...
  <div class="kpi-card bad"><div class="label">Meest afwijkend</div><div class="value">{names[worst_idx]}<br><span style="font-size:16px">{mapes[worst_idx]:.1%}</span></div></div>
    ...
    def _build_kpi_cards_from_hist_accuracy(self) -> str:
        ...
        sanity_cls = (
            "good" if sanity_pct <= 0.05 else ("warn" if sanity_pct <= 0.15 else "bad")
        )
  ...
  <div class="kpi-card bad"><div class="label">Meest afwijkend</div><div class="value">{worst}<br><span style="font-size:16px">{prog_mapes[worst]:.1%}</span></div></div>
    ...
    def _build_individual_kpi_cards(self) -> str:
        ...
        med_cls = (
            "good"
            if median_mape <= 0.10
            else ("warn" if median_mape <= 0.25 else "bad")
        )
        ratio = below_10 / total if total else 0
        below_cls = "good" if ratio >= 0.70 else ("warn" if ratio >= 0.40 else "bad")
  ...
  <div class="kpi-card bad"><div class="label">Meest afwijkend</div><div class="value">{worst}<br><span style="font-size:16px">{prog_mapes[worst]:.1%}</span></div></div>
    ...
    def _conversion_bar_chart(self) -> go.Figure | None:
        ...
        for idx, yr in enumerate(years):
            ...
            preds = [
                yr_data.loc[p, "predicted"]
                if p in yr_data.index and pd.notna(yr_data.loc[p, "predicted"])
                else 0
                for p in programmes
            ]
            actuals = [
                yr_data.loc[p, "actual"]
                if p in yr_data.index and pd.notna(yr_data.loc[p, "actual"])
                else 0
                for p in programmes
            ]
            conv_pct = [
                f"{a / p:.0%}" if p > 0 else "–" for p, a in zip(preds, actuals)
            ]
            ...
            buttons.append(
                dict(
                    ...
                    args=[
                        {"visible": vis},
                        {"title": f"Vooraanmelders vs inschrijvingen — {int(yr)}"},
                    ],
                )
            )
    ...
    def _model_accuracy_heatmaps(
        ...
        for col in all_mape_cols:
            ...
            fig.update_layout(
                ...
                xaxis_title="Collegejaar",
                yaxis_title="Opleiding",
                height=max(400, len(programmes) * 30 + 150),
            )
            results.append(
                (
                    f"Modelnauwkeurigheid {display}",
                    fig,
    ...
    def _accuracy_table(self) -> go.Figure | None:
        ...
        model_cols = [
            c
            for c in ("SARIMA_cumulative", "Prognose_ratio")
        ...
        for idx, yr in enumerate(years):
            ...
            buttons.append(
                dict(
                    ...
                    args=[
                        {"visible": vis},
                        {"title": f"Voorspelling vs realisatie — {int(yr)}"},
                    ],
                )
            )
    ...
    def _weekly_yoy_heatmap(self) -> go.Figure | None:
        ...
        merged = cur_agg.merge(
            prev_agg,
            on=["Croho groepeernaam", "Weeknummer"],
            suffixes=("_cur", "_prev"),
            how="inner",
        )
        merged["pct_change"] = (
        ...
        hover = pivot.map(lambda v: f"{v:+.1%}" if pd.notna(v) else "").values
    ...
    def _error_progression(
        ...
        for col in mape_cols:
            ...
            fig.add_trace(
                go.Scatter(
                    x=weekly["Weeknummer"].astype(str),
                    y=weekly[col],
                    mode="lines+markers",
                    name=_display(model),
    ...
    def _model_performance_summary(
        self,
        mape_cols: list[str],
        mae_cols: list[str] | None = None,
    ) -> go.Figure | None:
        """Summary table with overall metrics, per-herkomst, per-size, and bias."""
        hist = self._hist_data()
        valid_mape = (
            [c for c in mape_cols if c in hist.columns] if not hist.empty else []
        )
        ...
        if "Aantal_studenten" in hist.columns:
            ...
            for model, mc in zip(models, valid_mape):
                ...
                if pred_col not in hist.columns:
                    continue
                sub = hist[
                    hist[pred_col].notna() & hist["Aantal_studenten"].notna()
                ].copy()
                if sub.empty:
                    continue
                ...
                vals_row = [
                    f"{mean_bias:+.1f} studenten ({over_pct:.0%} overschatting)"
                ]
        ...
        if self.data_studentcount is not None and "Aantal_studenten" in hist.columns:
            ...
            prog_years = (
                hist.groupby(["Croho groepeernaam", "Collegejaar"])
                .first()
                .reset_index()
            )
            sc = self.data_studentcount
            ...
            if naive_errors:
                ...
                for model in models:
                    if baseline_errors[model]:
                        ...
                        vals.append(
                            f"{model_mape:.1%} vs {naive_mape:.1%} ({diff:+.1%})"
                        )
                    else:
                        vals.append("–")
        ...
        cells_by_col = (
            list(zip(*rows_section)) if rows_section else [[] for _ in header_labels]
        )
        ...
        fig.update_layout(
            height=max(350, 50 + 28 * len(rows_section)),
    ...
    def _model_performance_summary_from_studentcount(self) -> go.Figure | None:
        ...
        fills.append("#e8edf2")
        prog_volatility = (
            sc.groupby("Croho groepeernaam")["Aantal_studenten"].std()
            / sc.groupby("Croho groepeernaam")["Aantal_studenten"].mean()
        )
        ...
        if not prog_volatility.empty:
            rows_data.append(
                [
                    "Meest stabiel",
                    f"{prog_volatility.index[0]} ({prog_volatility.iloc[0]:.1%} CV)",
                ]
            )
            fills.append("white")
            rows_data.append(
                [
                    "Meest volatiel",
                    f"{prog_volatility.index[-1]} ({prog_volatility.iloc[-1]:.1%} CV)",
                ]
            )
            fills.append("white")
        ...
        fig.update_layout(
            height=max(300, 50 + 28 * len(rows_data)),
            margin=dict(l=10, r=10, t=30, b=10),
        )
        return fig
    ...
    def _predicted_vs_actual_scatter(self, pred_col: str) -> go.Figure | None:
        ...
        if agg.empty:
            return None
        agg["error_pct"] = (
            abs(agg["predicted"] - agg["actual"]) / agg["actual"] * 100
        ).clip(upper=30)
        ...
        fig.update_layout(
            ...
            xaxis_title="Voorspelde inschrijvingen",
            yaxis_title="Werkelijke inschrijvingen",
            ...
            buttons.append(
                dict(
                    ...
                    args=[
                        {"visible": vis},
                        {
                            "title": f"Voorspelfout per opleiding ({_display(pred_col)}) — {int(yr)}"
                        },
                    ],
                )
            )
    ...
    def _error_heatmap_animated(self, pred_col: str) -> go.Figure | None:
        ...
        actual = tmp["Aantal_studenten"].replace(0, np.nan)
        tmp["_mape"] = (
            abs(tmp[pred_col].fillna(0) - tmp["Aantal_studenten"]) / actual
        ).clip(upper=2.0)
    ...
    def _prediction_vs_actual(self) -> go.Figure | None:
        ...
        for i, prog in enumerate(programmes):
            ...
            if actual is not None and not actual.empty:
                fig.add_trace(
                    go.Scatter(
                        x=actual["Weeknummer"].astype(str),
                        y=actual["Gewogen vooraanmelders"],
                        mode="lines+markers",
                        name="Actuele vooraanmeldingen",
                ...
                fig.add_trace(
                    go.Scatter(
                        x=[], y=[], visible=visible, name="Actuele vooraanmeldingen"
                    )
                )
            ...
            fig.add_trace(
                go.Scatter(
                    x=week_agg["Weeknummer"].astype(str),
                    y=week_agg[pred_col],
                    mode="lines+markers",
            ...
            vis[i * traces_per_prog : (i + 1) * traces_per_prog] = [
                True
            ] * traces_per_prog
    ...
    def _prediction_stability(self) -> go.Figure | None:
        ...
        fig.update_layout(
            ...
            xaxis_title="Weeknummer",
            yaxis_title="Verandering in voorspelling",
            height=450,
            updatemenus=[
                dict(
                    active=0,
    ...
    def _cumulative_pipeline_chart(self) -> go.Figure | None:
        ...
        for prog in programmes:
            ...
            if not known.empty:
                ...
                known_agg = known_agg[
                    known_agg["Weeknummer"].apply(
                        lambda w: _week_sort_key(int(w)) <= pw_key
                    )
                ]
                known_agg = known_agg.sort_values(
                    "Weeknummer", key=lambda s: s.apply(_week_sort_key)
                )
            ...
            if sc is not None:
                real = sc[
                    (sc["Croho groepeernaam"] == prog)
                    & (sc["Collegejaar"] == prev_year)
                ]["Aantal_studenten"].sum()
                if real > 0:
                    idx = len(fig.data)
                    fig.add_trace(
    ...
    def _individual_prediction_vs_actual(self) -> go.Figure | None:
        ...
        if hist.empty or pred_col not in hist.columns:
            return None
        if (
            "Aantal_studenten" not in hist.columns
            or hist["Aantal_studenten"].dropna().empty
        ):
            return None
        ...
        for prog in programmes:
            ...
            for j, yr in enumerate(years):
                ...
                fig.add_trace(
                    go.Scatter(
                        x=week_agg["Weeknummer"].astype(str),
                        y=week_agg["predicted"],
                        mode="lines+markers",
                        name=f"{int(yr)} voorspelling",
                ...
                if pd.notna(actual_val) and actual_val > 0:
                    ...
                    fig.add_trace(
                        go.Scatter(
                            x=[
                                week_agg["Weeknummer"].astype(str).iloc[0],
                                week_agg["Weeknummer"].astype(str).iloc[-1],
                            ],
                            y=[actual_val, actual_val],
                            mode="lines",
                            name=f"{int(yr)} realisatie",
        ...
        fig.update_layout(
            ...
            xaxis_title="Weeknummer",
            yaxis_title="Voorspeld aantal studenten",
            height=550,
            hovermode="x unified",
            updatemenus=[
                dict(
    ...
    def _individual_error_by_herkomst(self) -> go.Figure | None:
        ...
        grouped = (
            hist.groupby("Herkomst")[mape_col]
            .agg(["mean", "std", "count"])
            .reset_index()
        )
        ...
        fig.add_trace(
            go.Bar(
                ...
                text=[
                    f"{m:.1%} (n={int(c)})"
                    for m, c in zip(grouped["mean"], grouped["count"])
                ],
                textposition="outside",
    ...
    def _individual_cockpit(self) -> go.Figure | None:
        ...
        pred = (
            ...
            if self.predict_week is not None
            else self.data[self.data["Collegejaar"] == self.prediction_year]
        )
        ...
        for prog in programmes:
            ...
            if real > 0:
                ...
                delta_colors.append(
                    "#2ca02c"
                    if ad <= 0.05
                    else ("#f0ad4e" if ad <= 0.15 else "#d62728")
                )
            else:
                delta_vals.append("\u2013")
        ...
        if total_real > 0:
            ...
            atd = abs(total_d)
            total_d_color = (
                "#2ca02c" if atd <= 0.05 else ("#f0ad4e" if atd <= 0.15 else "#d62728")
            )
        else:
            total_d_str = "\u2013"
        ...
        fig = go.Figure(
            go.Table(
                header=dict(
                    values=[
                        "Opleiding",
                        f"Prognose {self.prediction_year}",
                        f"Realisatie {prev_year}",
                        "\u0394%",
                    ],
                    fill_color="#1f77b4",
    ...
    def _individual_enrollment_history(self) -> go.Figure | None:
        ...
        agg = (
            sc.groupby(["Collegejaar", "Croho groepeernaam"])["Aantal_studenten"]
            .sum()
            .reset_index()
        )
        years = sorted(agg["Collegejaar"].unique())
    ...
    def _individual_herkomst_prediction(self) -> go.Figure | None:
        ...
        for herkomst in ("NL", "EER", "Niet-EER"):
            ...
            for prog in programmes:
                v = pred[
                    (pred["Croho groepeernaam"] == prog)
                    & (pred["Herkomst"] == herkomst)
                ][pred_col].sum()
        ...
        fig.update_layout(
            ...
            xaxis_title="Opleiding",
            yaxis_title="Voorspelde inschrijvingen",
            barmode="stack",
            height=max(400, len(programmes) * 30 + 200),
            xaxis=dict(tickangle=-45),
        )
    ...
    def _individual_kpi_cards_current(self) -> str:
        ...
        total_prev = sum(
            sc[(sc["Croho groepeernaam"] == p) & (sc["Collegejaar"] == prev_year)][
                "Aantal_studenten"
            ].sum()
            for p in programmes
        )
  ...
  <div class="kpi-card {"warn" if decliner[1] < -0.05 else "good"}"><div class="label">Meeste krimp</div><div class="value">{decliner[0]}<br><span style="font-size:16px">{decliner[1]:+.1%}</span></div></div>
    ...
    def _save_individual(self) -> None:
        pred_col = "SARIMA_individual"
        mape_cols = [
            c for c in self.data.columns if c.startswith("MAPE_") and "individual" in c
        ]
        ...
        if fig:
            charts.append(
                (
                    "Verwachte instroom per herkomst",
                    fig,
        ...
        if fig:
            charts.append(
                (
                    "Voorspelling vs Realisatie per opleiding",
                    fig,
        ...
        if fig:
            charts.append(
                (
                    "Voorspelling vs Realisatie (tabel)",
                    fig,
        ...
        if fig:
            charts.append(
                (
                    "Voorspeld vs Realisatie (scatter)",
                    fig,
    ...
    def _yearly_trend(self) -> go.Figure | None:
        ...
        for prog in programmes:
            ...
            for j, yr in enumerate(hist_years):
                yr_agg = (
                    ...
                    .groupby("Weeknummer")["Gewogen vooraanmelders"]
                    .sum()
                    .reset_index()
                    .sort_values("Weeknummer", key=_sort_weeks_series)
                )
                ...
                fig.add_trace(
                    go.Scatter(
                        x=yr_agg["Weeknummer"].astype(str),
                        y=yr_agg["Gewogen vooraanmelders"],
                        mode="lines",
                        name=str(int(yr)),
                        line=dict(
                            color=PASTEL_YEAR_COLOURS[j % len(PASTEL_YEAR_COLOURS)],
            ...
            if self.predict_week is not None:
                ...
                fig.add_trace(
                    go.Scatter(
                        x=before_pw["Weeknummer"].astype(str),
                        y=before_pw["Gewogen vooraanmelders"],
                        mode="lines+markers",
                        name=f"{self.prediction_year} (actueel)",
                        line=dict(color="#1f77b4", width=3),
                        visible=False,
                    )
                )
                ...
                if not after_pw.empty:
                    ...
                    fig.add_trace(
                        go.Scatter(
                            x=after_pw["Weeknummer"].astype(str),
                            y=after_pw["Gewogen vooraanmelders"],
                            mode="lines",
                            name=f"{self.prediction_year} (na voorspelweek)",
                ...
                fig.add_trace(
                    go.Scatter(
                        x=full_agg["Weeknummer"].astype(str),
                        y=full_agg["Gewogen vooraanmelders"],
                        mode="lines+markers",
                        name=f"{self.prediction_year} (actueel)",
                        line=dict(color="#1f77b4", width=3),
                        visible=False,
            ...
            if "Voorspelde vooraanmelders" in self.data.columns:
                sarima_sub = (
                    ...
                    .groupby("Weeknummer")["Voorspelde vooraanmelders"]
                    .sum()
                    .reset_index()
                ...
                if self.predict_week is not None:
                    ...
                    sarima_sub = sarima_sub[
                        sarima_sub["Weeknummer"].apply(
                            lambda w: _week_sort_key(int(w)) > pw_key
                        )
                    ]
                sarima_sub = sarima_sub.sort_values(
                    "Weeknummer", key=_sort_weeks_series
                )
                    ...
                    bridge = pd.DataFrame(
                        {
                            ...
                            "Voorspelde vooraanmelders": [
                                known.iloc[-1]["Gewogen vooraanmelders"]
                            ],
                        }
                    )
                ...
                if not sarima_sub.empty:
                    ...
                    fig.add_trace(
                        go.Scatter(
                            x=sarima_sub["Weeknummer"].astype(str),
                            y=sarima_sub["Voorspelde vooraanmelders"],
                            mode="lines",
                    ...
                    if hist_years and self.predict_week is not None:
                        hw = (
                            ...
                            .groupby(["Weeknummer", "Collegejaar"])[
                        ...
                        std_map = (
                            hw.groupby("Weeknummer")["Gewogen vooraanmelders"]
                            .std()
                            .fillna(0)
                        )
                        ...
                        band["lower"] = (
                            band["Voorspelde vooraanmelders"] - band["std"]
                        ).clip(lower=0)
                    ...
                    if total > 0:
                        ...
                        fig.add_trace(
                            go.Scatter(
                                ...
                                y=[total],
                                mode="markers+text",
                                name="Actuele inschrijvingen"
                                if first_star
                                else f"Realisatie {int(yr)}",
                                marker=dict(
                                    symbol="star",
                                    size=14,
            ...
            if best_col is not None and self.predict_week is not None:
                ...
                if not pred_pw.empty:
                    ...
                    if (
                        "Voorspelde vooraanmelders" in self.data.columns
                        and not sarima_38.empty
                    ):
        ...
        if programmes:
            for trace_idx, default_vis in prog_traces[programmes[0]]:
                fig.data[trace_idx].visible = (
                    "legendonly" if default_vis == "legendonly" else True
                )

        fig.update_layout(
            title="Verloop per opleiding — alle jaren",
            xaxis_title="Weeknummer",
            yaxis_title="Gewogen vooraanmelders",
            height=600,
            updatemenus=[
                dict(
                    active=0,
    ...
    def _save_cumulative(self) -> None:
        ...
        if fig:
            charts.append(
                (
                    "Verloop per opleiding — alle jaren",
                    fig,
        ...
        if fig:
            charts.append(
                (
                    "Vooraanmelders vs inschrijvingen",
                    fig,
        ...
        if fig:
            charts.append(
                (
                    "Modelvergelijking per opleiding",
                    fig,
        ...
        for pc in ("SARIMA_cumulative", "Prognose_ratio"):
            ...
            if fig:
                charts.append(
                    (
                        f"Voorspeld vs Realisatie ({_display(pc)})",
                        fig,
                        "Elke bol is een opleiding in een bepaald jaar. Bolgrootte toont het werkelijke aantal studenten. "
        ...
        for pc in ("SARIMA_cumulative", "Prognose_ratio"):
            ...
            if fig:
                charts.append(
                    (
                        f"Voorspelfout per opleiding ({_display(pc)})",
                        fig,
    ...
    def _cockpit_with_confidence(self) -> go.Figure | None:
        ...
        hist = self._hist_data()
        all_mape_cols = (
            [c for c in hist.columns if c.startswith("MAPE_")] if not hist.empty else []
        )
        ...
        for prog in programmes:
            ...
            if (
                avg_mape is None
                and self.data_studentcount is not None
                and self.data_cumulative is not None
            ):
                dc = self.data_cumulative
    ...
    def _prognose_vs_vorig_jaar(self) -> go.Figure | None:
        ...
        for i, prog in enumerate(programmes):
            ...
            if actual is not None and not actual.empty:
                fig.add_trace(
                    go.Scatter(
                        ...
                        y=actual["Gewogen vooraanmelders"],
                        mode="lines+markers",
                        name=f"Actueel {self.prediction_year}",
                        line=dict(color="cyan"),
                        visible=visible,
                    )
                )
            ...
            if self.data_cumulative is not None:
                ...
                fig.add_trace(
                    go.Scatter(
                        x=prev["Weeknummer"].astype(str),
                        y=prev["Gewogen vooraanmelders"],
                        mode="lines",
                        name=f"Realisatie {prev_year}",
                        line=dict(color="black", dash="dash"),
                        visible=visible,
                ...
                fig.add_trace(
                    go.Scatter(
                        x=[], y=[], visible=visible, name=f"Realisatie {prev_year}"
                    )
                )
    ...
    def _origin_stacked_bar(self) -> go.Figure | None:
        ...
        if self.predict_week is not None:
            pw_key = _week_sort_key(self.predict_week)
            cur = cur[
                cur["Weeknummer"].apply(lambda w: _week_sort_key(int(w)) <= pw_key)
            ]
        ...
        for herkomst in ("NL", "EER", "Niet-EER"):
            ...
            vals = [
                cur_max.loc[
                    (cur_max["Croho groepeernaam"] == p)
                    & (cur_max["Herkomst"] == herkomst),
                    "Gewogen vooraanmelders",
                ].sum()
                for p in programmes
            ]
            ...
            vals_prev = [
                prev_max.loc[
                    (prev_max["Croho groepeernaam"] == p)
                    & (prev_max["Herkomst"] == herkomst),
                    "Gewogen vooraanmelders",
                ].sum()
                for p in programmes
            ]
    ...
    def _numerus_fixus_bars(self) -> go.Figure | None:
        ...
        for prog, cap in self.numerus_fixus_list.items():
            ...
            if self.data_cumulative is not None and self.predict_week is not None:
                ...
                if not sub_dc.empty:
                    current = (
                        sub_dc.groupby("Weeknummer")["Gewogen vooraanmelders"]
                        .sum()
                        .max()
                    )
    ...
    def _final_kpi_cards(self) -> str:
        ...
        if total_prev > 0:
            ...
            yoy_cls = (
                "good" if yoy_pct >= 0 else ("warn" if yoy_pct >= -0.05 else "bad")
            )
            yoy_val = f"{yoy_pct:+.1%}"
  ...
  <div class="kpi-card {nf_cls}"><div class="label">Numerus fixus boven capaciteit</div><div class="value">{nf_over} van {nf_total}</div></div>"""
    ...
    def _tuning_summary_html(self) -> str:
        """Render tuning cache info as an HTML card for the dashboard."""
        from studentprognose.tuning.cache import _load_cache
        ...
        rows = []
        for model_key in ("xgboost_classifier", "xgboost_regressor"):
            entry = cache.get(model_key)
            if entry is None:
                continue
            params = entry.get("params", {})
            score = entry.get("cv_score", "?")
            n_folds = entry.get("n_folds", "?")
            n_combos = entry.get("n_combinations", "?")
            ...
            metric = "MAPE" if "classifier" in model_key else "MAE"
            score_fmt = (
                f"{score:.1%}"
                if isinstance(score, float) and "classifier" in model_key
                else f"{score:.1f}"
            ...
            rows.append(
                f"<tr><td><b>{label}</b></td><td>{metric}: {score_fmt}</td>"
                f"<td>{n_folds} folds, {n_combos} combinaties</td>"
                f"<td style='font-size:0.85em'>{param_str}</td>"
                f"<td style='font-size:0.85em'>{ts}</td></tr>"
            )

        sarima_orders = cache.get("sarima_orders", {})
        if sarima_orders:
            n_orders = len(sarima_orders)
            ts = cache.get("sarima_orders_timestamp", "?")[:19]
            rows.append(
                f"<tr><td><b>SARIMA</b></td><td>AIC-selectie</td>"
                f"<td>{n_orders} tijdreeksen</td>"
                f"<td style='font-size:0.85em'>Per opleiding/herkomst/examentype</td>"
                f"<td style='font-size:0.85em'>{ts}</td></tr>"
            )
        ...
        return (
            '<div style="margin:24px 0;padding:16px;background:#f8f9fa;border-radius:8px;border:1px solid #dee2e6">'
            '<h3 style="margin-top:0">Hyperparameter Tuning</h3>'
            '<table style="width:100%;border-collapse:collapse;font-size:0.95em">'
            '<tr style="border-bottom:2px solid #dee2e6">'
            '<th style="text-align:left;padding:6px">Model</th>'
            '<th style="text-align:left;padding:6px">Score</th>'
            '<th style="text-align:left;padding:6px">CV</th>'
            '<th style="text-align:left;padding:6px">Parameters</th>'
            '<th style="text-align:left;padding:6px">Getuned op</th></tr>'
            + "".join(rows)
            + "</table></div>"
        )
    ...
    def _save_final(self) -> None:
        ...
        kpi = (
            ...
            + self._final_kpi_cards()
            + self._tuning_summary_html()
        )
