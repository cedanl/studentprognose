...
!!! info "Dashboard is opt-in"
    ...
    **Fout-isolatie per pagina.** Elke dashboard-pagina (individual, cumulative, final) wordt onafhankelijk opgebouwd: een fout in één pagina laat de andere twee gewoon doorlopen. De log bevat in dat geval per pagina een aparte sectie (bijv. `=== cumulative ===`), en stdout toont per pagina een gerichte waarschuwing zoals `Waarschuwing: dashboard 'cumulative' niet gegenereerd (fout opgeslagen in …)`. Crasht de dashboard-stap nog vóór de pagina-lus (bijvoorbeeld bij het opbouwen van de data), dan komt die stack trace onder een `=== fatal ===` sectie te staan; de log wordt in append-modus geschreven zodat een late fout de per-pagina secties nooit overschrijft.

    **Expliciete skip-reden.** Een pagina wordt alleen gegenereerd als het bijbehorende spoor daadwerkelijk een prognose voor het voorspeljaar heeft geleverd — dus een niet-lege (niet-NaN) waarde in `SARIMA_individual` voor het individuele spoor, of in `SARIMA_cumulative`/`Prognose_ratio` voor het cumulatieve spoor. Levert een spoor niets op, dan zie je in stdout een melding als `Dashboard 'cumulative' overgeslagen: het cumulatieve spoor heeft geen prognose voor 2025 geleverd — geen waarden in ('SARIMA_cumulative', 'Prognose_ratio').`. Geen stille skip meer. Let op: in een `-d both` run zet het individuele spoor de kolom `SARIMA_cumulative` altijd klaar (gevuld met NaN), dus de tool kijkt bewust naar de *waarden* voor het voorspeljaar en niet naar het enkel bestaan van de kolom.
